...
describe(zctas_full$final_wtd_as_avg20092011)  # n=1572 available
...
zctas_final <- zctas_full %>%
  ...
  mutate(
    reduce_NA = if_else((c(wtd_as_avg20002005 - final_wtd_as_avg20092011) > 0.5) &
                          (c(wtd_as_avg20002005 - final_wtd_as_avg20092011) < 1), 1, 0), #have to account for the values between 0.5-1
    reduce_1 = if_else(c(wtd_as_avg20002005 - final_wtd_as_avg20092011) >= 1, 1, 0),
    reduce_2 = if_else(c(wtd_as_avg20002005 - final_wtd_as_avg20092011) >= 2, 1, 0),                
    reduce_3 = if_else(c(wtd_as_avg20002005 - final_wtd_as_avg20092011) >= 3, 1, 0),
    compiled_lower = case_when(reduce_NA == 1 ~ 0, #mutually exclusive (0.5, 0.5-1, >= 1)
                               reduce_1 == 0 ~ 1,
                               reduce_1 == 1 ~ 2), 
    compiled_lower = if_else(reduce_2 == 1, 3, compiled_lower), #overwrite when not mutually exclusive
    compiled_lower = if_else(reduce_3 == 1, 4, compiled_lower)) %>%
...
mypal3<-c("white","#fef0d9","#fdbb84","#b30000","#851515")
...
plot_as_ca <-ggplot() + 
  ...
  scale_fill_manual(values=mypal3,
                    ...
                    breaks=c(0,1,2,3,4),
                    labels=c("N/A","No reduction", "≥ 1", "≥ 2", "≥ 3"))+
...
#ggsave("map_magnitude.png", plot = last_plot(), dpi = 300)
