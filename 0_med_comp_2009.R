# File: Medical complications set up - 2009
# Date: 11/30/2023
...
bsmf_2009 <- readr::read_fwf(here("data", "raw", "2009_2010","Nigra_BSMF_2009.dat"),
                             fwf_cols(ch_sex=c(58), plural=c(61), plural_ord=c(62),
                                      ...
                                      birth_countyplace=c(79, 80), mom_birthplace=c(97, 99), 
...
bsmf_2009 <- bsmf_2009 %>% 
  mutate(last_liv_date = str_replace(last_liv_date, "(^\\s+)|(\\s+$)", ""), #get rid of breakspace at beginning and end
         ...
         last_liv_date = ifelse(as.numeric(last_liv_date) > 2010, NA, last_liv_date)) #removes errors after dataset date
...
bsmf <- bind_cols(bsmf_2009, preg_2009, labor_2009)

write.csv(bsmf, file= here("data","intermediate","bsmf_2009_int.csv"))