# File: Medical complications set up - 2002
# Date: 11/30/2023
...
bsmf_2002 <- readr::read_fwf(here("data", "raw", "2000_2005","Nigra_BSMF_2002.dat"),
...
bsmf_2002 <- bsmf_2002 %>% 
  mutate(last_liv_date = str_replace(last_liv_date, "(^\\s+)|(\\s+$)", ""), #get rid of breakspace at beginning and end
         ...
         last_liv_date = ifelse(as.numeric(last_liv_date) > 2003, NA, last_liv_date)) #removes errors after dataset date
...
bsmf <- bind_cols(bsmf_2002, preg_2002, labor_2002)

write.csv(bsmf, file= here("data","intermediate","bsmf_2002_int.csv"))