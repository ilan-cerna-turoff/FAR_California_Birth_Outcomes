# File: Medical complications set up - 2010
# Date: 11/30/2023
...
bsmf_2010 <- readr::read_fwf(here("data", "raw", "2009_2010","Nigra_BSMF_2010.dat"),
...
bsmf_2010 <- bsmf_2010 %>% 
  mutate(last_liv_date = str_replace(last_liv_date, "(^\\s+)|(\\s+$)", ""), #get rid of breakspace at beginning and end
         ...
         last_liv_date = ifelse(as.numeric(last_liv_date) > 2011, NA, last_liv_date)) #removes errors after dataset date
...
bsmf <- bind_cols(bsmf_2010, preg_2010, labor_2010)

write.csv(bsmf, file= here("data","intermediate","bsmf_2010_int.csv"))