# File: Medical complications set up - 2008
# Date: 11/30/2023
...
bsmf_2008 <- readr::read_fwf(here("data", "raw", "2007_2008","Nigra_BSMF_2008.dat"),
...
bsmf_2008 <- bsmf_2008 %>% 
  mutate(last_liv_date = str_replace(last_liv_date, "(^\\s+)|(\\s+$)", ""), #get rid of breakspace at beginning and end
         ...
         last_liv_date = ifelse(as.numeric(last_liv_date) > 2009, NA, last_liv_date)) #removes errors after dataset date
...
bsmf <- bind_cols(bsmf_2008, preg_2008, labor_2008)

write.csv(bsmf, file= here("data","intermediate","bsmf_2008_int.csv"))